...
The initial scaffold contained **11 explicit `sorry` occurrences**. On the current proof branch, the Kummer and minimal signed zero-sum gaps have been eliminated, leaving **5 explicit `sorry` occurrences**:

- `MinusOne.lean`: `mixed_case_one_borrow_of_gt`, `exceptional_mixed_no_one_borrow`, `exceptional_mixed_two_borrow_witness`, `minus_one_valuation`.
