...
1. **Kummer / carries.** Mathlib provides `padicValNat_choose`, expressing the valuation of `Nat.choose` as a carry count. The local bridge `padicVal_choose_eq_zero_iff_digitwiseLE` is now proved via prefix remainders and absence of carries.
...
The initial scaffold contained **11 explicit `sorry` occurrences**. On the current proof branch, the Kummer gap has been eliminated, leaving **10 explicit `sorry` occurrences**:
